...
AMTableOfContents.prototype.Initialize = function()
{
	...
	else if(this.tocId)
	{
		...
		if (this.properties)
		{
			...
			{
				this.sectionName = this.properties.section.name;
				gAMReader.secColor = this.properties.section.color;
				result = true;
			}
		}
	} 
...
AMTableOfContents.prototype.InstallLayout = function()
{
	...
	if(!gAMReader.isPortrait)
		template = 'default_layout_landscape.html';
	else
		template = 'default_layout_portrait.html';
...
function AMHyperlinkHandle()
{
    ...
    for(var i = 0; i < elength; i++)
	{
		if( !elem[i].classList.contains('am_gallery') && !elem[i].classList.contains('close') )
		{
