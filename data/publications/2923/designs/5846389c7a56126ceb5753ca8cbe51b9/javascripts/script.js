...
function divideContent(body, ws, content, padding, docHeight, headerHeight, colWidth, isLandscape, captionHeight, panelWidth) {
    ...
    var k = 1;
    var check = true;
    while (k < lengthChild && allChild[k].offsetLeft < panelWidth && check) {
        if (allChild[k].nodeName.indexOf('DIV') < 0) {
    ...
    if (isLandscape) {
        ...
        while (k < lengthChild && allChild[k].offsetLeft < panelWidth && checkLandcape) {
            if (allChild[k].nodeName.indexOf('DIV') < 0 && allChild[k].className.indexOf('white-space') < 0) {
                if (allChild[k].offsetTop + allChild[k].offsetHeight > docHeight || allChild[k].offsetLeft > colWidth * 2) {
                    var condition = false;
                    if (allChild[k].className.indexOf('spanned') >= 0 && allChild[k].querySelectorAll("span")[allChild[k].querySelectorAll("span").length - 1].offsetLeft > colWidth * 2) {
                        currentElement = allChild[k];
                        condition = true;
                    } else {
                        if (allChild[k].offsetLeft > colWidth && allChild[k].className.indexOf("white-space") < 0) {
                            currentElement = allChild[k];
                            condition = true;
                        }
                    }
