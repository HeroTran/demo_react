function AMCustomScript(mode, fontSize, navigate, cache, cb, readerObj)
{
	try{
        ...
        //query container
        var container = body.querySelector(':scope > div:nth-of-type(2)');
		...
		styleByline(container);
		//run script for special template
		switch(templateID){
			case 'reviews':
			case 'reviews-editor-choice':
				configReviewTable(container);
				setImageReviewTable(container);
				setRatingDot(container);
				break;
			default:
				break;
		}
...
function styleByline(container){
	if(container){
		var byLine = container.querySelectorAll(':scope .head-info > .byline > span')[0];
		if(byLine){
			var str = byLine.innerText;
			var resBy = str.substring(0,2).trim().toLowerCase();
			var resName = str.substring(2,str.length).trim().toLowerCase();
			byLine.innerHTML = '<b>' + resBy + '</b>'+ ' ' + resName;
		}
	}
...
function configReviewTable(container){
	//config below head info
	var content = container.querySelectorAll(':scope .right_content')[0];
	var reviewTab;
	if(content){
		reviewTab = content.firstElementChild;
	}
	if(reviewTab.tagName =='DIV'){
		var temp = reviewTab.querySelectorAll(':scope table')[0]
		if(temp){
			temp.className += ' custom-table';
		}
	}
}
function setImageReviewTable(container){
	var imageReviewTableList = container.querySelectorAll(':scope .right_content table[bgcolor="#eeeeee"] td > .image');
	var tmpDataRel, tmpSpan, tmpImg;
	var newSpan = document.createElement('span');
	newSpan.className += 'review-icon';
	if (imageReviewTableList.length > 0){
		for (var i = 0; i < imageReviewTableList.length; i++) {
			tmpSpan = newSpan.cloneNode(true);
			tmpImg = imageReviewTableList[i].querySelector('img');
			if(tmpImg){
				tmpDataRel = tmpImg.getAttribute('data-rel');
			}
			//set data-rel, link for img in review table
			tmpSpan.setAttribute('data-rel', tmpDataRel);
			imageReviewTableList[i].appendChild(tmpSpan);
			tmpImg.setAttribute('data-rel','');
		}
	}
}
function setRatingDot(container){
	var valueRatList = container.querySelectorAll(':scope .right_content table[bgcolor="#eeeeee"] td .rating');
	var tmpVal, tmpSpan, digits, fraction, spanDot, spanHalfDot, spanBlankDot;
	var i,k,numberCountDot;
	spanDot = document.createElement('span');
	spanDot.className += 'dot';
	spanHalfDot = document.createElement('span');
	spanHalfDot.className += 'half-dot';
	spanBlankDot = document.createElement('span');
	spanBlankDot.className += 'blank-dot';
	if(valueRatList.length > 0){
		numberDot = -1;
		for (i = valueRatList.length - 1; i >= 0; i--) {
			var tmp = valueRatList[i].textContent;
			var values = tmp.split(".");
			var floatNum = parseFloat(tmp);
			digits = parseInt(values[0]);
			fraction = parseInt(values[1]);
			valueRatList[i].textContent = '';
			if(!floatNum || floatNum <= 0){
				for (k = 0; k < 5;k++) {
					tmpSpan = spanBlankDot.cloneNode(true);
					valueRatList[i].appendChild(tmpSpan);
				}
			}else if(floatNum >=5){
				for (k = 0; k < digits;k++) {
					tmpSpan = spanDot.cloneNode(true);
					valueRatList[i].appendChild(tmpSpan);
				}
			}else{
				//create dot with digits
				for (k = 0; k < digits;k++) {
					tmpSpan = spanDot.cloneNode(true);
					valueRatList[i].appendChild(tmpSpan);
					numberDot += 1;
				...
				if(fraction){
					//create half dot with digits
					tmpSpan = spanHalfDot.cloneNode(true);
					valueRatList[i].appendChild(tmpSpan);
					numberDot += 1;
				}
				if(numberDot < 4){
			...
			var prevSib = valueRatList[i].previousSibling;
			prevSib.appendChild(valueRatList[i]);
		}
	}
}
