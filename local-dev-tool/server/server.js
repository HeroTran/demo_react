...
module.exports = function(app){
    ...
    app.get('/sites/:siteId/issues/:issueId',function(req,res){
        issuesApi.get(req.params.siteId, req.params.issueId, parseInt(req.query.mobile)).then(function(data){
            res.json(data);
        })
    });

    app.post('/sites/:siteId/issues/:issueId',function(req,res){
        issuesApi.add(req.params.siteId, req.params.issueId, parseInt(req.query.mobile)).then(function(data){
            res.json(data);
        })
    });

    app.delete('/sites/:siteId/issues/:issueId',function(req,res){
        issuesApi.remove(req.params.siteId, req.params.issueId, parseInt(req.query.mobile));
        res.json({[req.params.issueId]:'deleted'});
        
    });

    app.get('/sites/:siteId/issues',function(req,res){
        issuesApi.query(req.params.siteId, parseInt(req.query.mobile)).then(function(data){
            res.json(data);
        })
    });
