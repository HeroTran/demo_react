...
Reader.prototype.initialize = function()
{
	...
	this.minHeightGallery = getQueryString('minHeightGallery', 750);
	this.minWidthGallery = getQueryString('minWidthGallery', 250);
	...
	var storageObj = {
		...
		minHeightGallery: getQueryString('minHeightGallery', 750),
		minWidthGallery: getQueryString('minWidthGallery', 250),
...
Reader.prototype.loadData = function(){
	var count = 2, properties = '', stories = '';
	var finish = function(){
	...
	var _this = this;
	this.fetchJSON('../'+this.path+this.issueID+'/toc.js', function(res){
		properties = _this.findSection(_this.tocID, res);
		counter();
	});
	this.fetchJSON('../'+this.path+this.issueID+'/stories.js', function(res){
		stories = res;
		counter();
	});
}
...
Reader.prototype.renderLayout = function()
{
    ...
    if( this.storyId ) 
	{
		story = this.findStory(this.storyId, this.stories);
		if(story.layout && story.layout[this.orientation]){
			layout = story.layout[this.orientation].link;
			if(!layout || layout.indexOf('twig') < 0){
				layout = this.setDefaultLayout(this.orientation);
			}
		}
		else{
			layout = this.setDefaultLayout(this.orientation);
	...
	{
		story = this.properties.stories;
		layout = this.properties[this.orientation + '_view'];
	}
	...
	for(var i=bylines.length-1; i>=0; i--){
		sByline += '<span>' + bylines[i].innerText + '</span>&nbsp;';
		...
		if(src.indexOf(this.storyId) < 0 && src.indexOf('http') < 0){
			...
			if(story.related_objects && story.related_objects.image){
				while(j < story.related_objects.image.length && cont){
					...
					if(rImg.portrait == isPortrait && rImg.image_url == src && ((rImg.width > this.minHeightGallery && rImg.height > this.minWidthGallery) || (rImg.width > this.minWidthGallery && rImg.height > this.minHeightGallery)) ){
    ...
    data = {
        ...
        section: {
            color: this.properties.layout.color,
...
Reader.prototype.setDefaultLayout = function(orientation)
{
	var layout = 'default_layout_portrait.twig';
...
Reader.prototype.findSection = function(sectionId, data)
{
    //console.log(jsel(data.issue.sections).select("//stories/*[@id='"+storyId+"']"))
    var dom = jsel(data);
    return dom.select("//section/*[@id='"+sectionId+"']");
}
...
function AMRender() {
	...
	window.reader.finishRender = function(){
		...
		if(window.reader.cache == null){
	        embededFunc = "<script src='js/reader3.js'></script>\
