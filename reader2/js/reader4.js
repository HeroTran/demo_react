...
Reader.prototype.loadData = function(){
	...
	this.fetchJSON('../'+this.path+this.issueID+'/toc-'+this.tocID+'.js', function(res){
		section = res;
		counter();
	});
	this.fetchJSON('../'+this.path+this.issueID+'/story-'+this.storyId+'.js', function(res){
		story = res;
		counter();
	});
}
...
Reader.prototype.renderLayout = function()
{
	...
	var sColor = (this.section.title_color) ? this.section.title_color : '';
	var sName = (this.section.section_name) ? this.section.section_name : '';
	var sDescription = (this.section.description) ? this.section.description : '';
    var data = {
        issue: {
            id: this.issueID,
			name: this.issueName
...
function AMRender() {
	...
	window.reader.finishRender = function(){
		...
		if(window.reader.cache == null){
	        embededFunc = "<script src='js/reader4.js'></script>\
